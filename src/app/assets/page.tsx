...
export default async function AssetsPage({ searchParams }: { searchParams: Promise<{ q?: string; kind?: string; status?: string }> }) {
  ...
  const [rows, insts] = await Promise.all([
    ...
    db.select({ id: instruments.id, externalId: instruments.externalId, model: instruments.model }).from(instruments),
  ]);
  ...
  const filtered = rows.filter((a) => {
    ...
    return [a.kind, a.model, a.serial, a.manufacturer, a.location, a.note, sys?.externalId ?? "", sys?.model ?? ""]
    ...
    <div className="container" style={{ maxWidth: 720 }}>
      <div className="card">
        ...
        <div className="mut" style={{ fontSize: 12, marginBottom: 10 }}>
          Every unit the systems are built from - attached, spare, or retired. Tap one for its full service history.
        </div>
        <AssetRegistryFilter q={q} kind={kind} status={status} kinds={[...MODULE_KINDS]} statuses={[...ASSET_STATES]} />
          ...
          return (
            <Link key={a.id} href={`/assets/${a.id}`} className="row-hover"
              ...
              <span className="mut" style={{ fontSize: 12, marginLeft: "auto" }}>
                {sys ? `in ${sys.externalId}` : a.status === "Decommissioned" ? "retired" : `Spare${a.location ? ` · ${a.location}` : ""}`}
              </span>

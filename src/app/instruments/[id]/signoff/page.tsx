...
import { db } from "@/db";
import { instruments, tasks, checklistItems, parts, attachments, assets, procedures, signoffs, taskResults} from "@/db/schema";
...
export default async function SignoffPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!houseOf(user, inst.tenantOrgId)) notFound();
    ...
    <div className="container page">
      ...
      <div className="card">
        <PrintHeader logoUrl={brand.operatorLogoUrl} operator={brand.operatorName}
          title="Instrument delivery & sign-off" date={shopMonthDay(new Date())} docId={inst.externalId} />

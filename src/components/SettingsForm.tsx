...
/** One settings group. Four of them, in the order you'd set an instance up. */
...
export default function SettingsForm(props: {
  clientAccessEnabled: boolean; allowlist: AllowRow[];
  stageDefs: StageRow[]; people: PersonRow[];
  ...
  // Organizations, their recipients, and the people who sign in as each.
  ...
  const [invite, setInvite] = useState<Record<number, { entry: string; role: string }>>({});
  ...
  const sendInvite = (orgId: number) => {
    const d = invite[orgId];
    if (!d?.entry.trim()) return;
    setError("");
    startTransition(async () => {
      const res = await addClientAccess(d.entry, orgId, d.role === "editor");
      if (res?.error) setError(res.error);
      else setInvite((m) => ({ ...m, [orgId]: { entry: "", role: d.role } }));
    });
  };
  ...
  /** One organization's sign-in list, with role and removal per person. */
  const peopleFor = (orgId: number) => props.allowlist.filter((r) => r.orgId === orgId);
  const orphans = props.allowlist.filter((r) => r.orgId === null);

  const signInRow = (r: AllowRow) => (
    <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0", flexWrap: "wrap" }}>
  ...
  return (
    <>
      ...
      {/* 2 - who's on the platform, and everything that belongs to them */}
      <Section title="Organizations"
        hint="A client owns systems; a provider services them. Each sees only what's shared with it, and each carries its own people, look and report recipients.">
        <div style={{ display: "flex", gap: 12, alignItems: "center", padding: "2px 0 10px" }}>
          ...
          <div>
            ...
            <div className="mut" style={{ fontSize: 11 }}>Master switch. Off blocks every non-staff sign-in, whatever the lists below say.</div>
          </div>
        </div>

        {props.orgs.map((o) => (
          <div key={o.id} style={{ border: "1px solid var(--line)", borderRadius: 10, padding: "10px 12px", marginBottom: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              ...
              {props.modules.sheetSync && props.sheetOrgId === o.id && (
                <span className="pill" style={{ background: "#E5F3E5", color: "#2E6B2E" }}>sheet sync</span>
              )}
              <span style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
                {/* Owner override: orgs style themselves on their own dashboard. */}
                ...
                {/* Only the sheet module still keys off this org. */}
                {props.modules.sheetSync && props.sheetOrgId !== o.id && o.kind === "client" && (
                  <button className="btn link" style={{ fontSize: 11 }} disabled={pending}
                    onClick={() => startTransition(() => setSheetOrg(o.id))}>use for sheet sync</button>
                )}
            ...
            <div style={{ borderTop: "1px solid var(--line)", marginTop: 8, paddingTop: 6 }}>
              {peopleFor(o.id).map(signInRow)}
              {peopleFor(o.id).length === 0 && (
                <div className="mut" style={{ fontSize: 11, paddingBottom: 4 }}>Nobody here can sign in yet.</div>
              )}
              <div style={{ display: "flex", gap: 6, marginTop: 4, flexWrap: "wrap" }}>
                <input className="mono" value={invite[o.id]?.entry ?? ""}
                  onChange={(e) => setInvite((m) => ({ ...m, [o.id]: { entry: e.target.value, role: m[o.id]?.role ?? "viewer" } }))}
                  onKeyDown={(e) => { if (e.key === "Enter") sendInvite(o.id); }}
                  placeholder="jane@company.com or @company.com" style={{ flex: "1 1 180px", fontSize: 12 }} />
                <select value={invite[o.id]?.role ?? "viewer"} style={{ width: "auto", fontSize: 12 }}
                  onChange={(e) => setInvite((m) => ({ ...m, [o.id]: { entry: m[o.id]?.entry ?? "", role: e.target.value } }))}>
                  <option value="viewer">viewer</option>
                  <option value="editor">editor</option>
                </select>
                <button className="btn sm accent" onClick={() => sendInvite(o.id)} disabled={pending || !invite[o.id]?.entry.trim()}>
                  {pending ? "..." : "Invite"}
                </button>
              </div>
            </div>
        ...
        {orphans.length > 0 && (
          <div style={{ border: "1px dashed #EAD9B0", background: "#FDF8EE", borderRadius: 10, padding: "10px 12px", marginBottom: 8 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#8A5410" }}>Not assigned to an organization</div>
            <div className="mut" style={{ fontSize: 11, marginBottom: 4 }}>
              These entries cannot sign in until they name one.
            </div>
            {orphans.map(signInRow)}
          </div>
        )}
      ...
      {/* 4 - names work can be assigned to */}
      <Section title="Roster"
        hint="Names tasks can be assigned to and @mentions can reach. An email makes assignments and mentions reach them; without one they can still be assigned, just not notified. Signing in is separate - that's the lists above.">
        {props.people.map((p) => (

...
type Asset = {
  ...
  location: string; note: string; status: string; instrumentId: number | null;
};

export default function AssetControls({ asset, systems, canEdit, isStaff }: {
  asset: Asset; systems: { id: number; externalId: string }[]; canEdit: boolean; isStaff: boolean;
}) {
  const [error, setError] = useState("");
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState({ kind: asset.kind, model: asset.model, serial: asset.serial, manufacturer: asset.manufacturer, location: asset.location, note: asset.note });
    ...
    <>
      ...
      {editing && (
        <div className="dash-form" style={{ marginTop: 10 }}>
          ...
          <div className="pf3" style={{ marginBottom: 10 }}>
            <div><label>Manufacturer</label><input value={draft.manufacturer} onChange={(e) => setDraft({ ...draft, manufacturer: e.target.value })} /></div>
            <div><label>Location (spares)</label><input value={draft.location} onChange={(e) => setDraft({ ...draft, location: e.target.value })} placeholder="shelf B" /></div>

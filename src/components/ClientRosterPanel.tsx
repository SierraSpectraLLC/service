...
import { heldOutOfFleet, stageOf, STAGE_TONE, STAGE_WORD } from "@/lib/orgStage";
import { DataTable, FacetStrip, Pill, Toolbar, type DataRow } from "@/components/ui";
import { toast } from "@/components/ui/Toast";

/** The facet strip's words. Plurals, because each one names a group. */
const FACET_LABEL: Record<string, string> = {
  client: "Clients", prospect: "Prospects", provider: "Providers", former: "Former",
};
  ...
  return (
    <>
      <Toolbar
        ...
        facets={
          /* The stages sit beside the two kinds because to a reader it is one
             question - which of these companies is this list about - and
             because a Clients facet that included the people we are still
             selling to, or the ones who have left, would be the roster telling
             the same lie the fleet was.

             Former clients last, and countless when there are none: a shop
             with no former clients does not need a word for them on screen,
             and a facet showing a bare zero is a filter that promises
             something and then shows an empty table. */
          <FacetStrip facets={(["client", "prospect", "provider", "former"] as const).map((k) => ({
            key: k,
            label: FACET_LABEL[k],
            count: rows.filter((o) => (k === "prospect" || k === "former"
              ? stageOf(o.stage) === k
              : o.kind === k && stageOf(o.stage) === "client")).length || undefined,
            on: filter.kind === k, href: href(filter.kind === k ? "" : k),
          })).filter((f) => f.key !== "former" || f.count || filter.kind === "former")} />
        }
      />
      ...
      <DataTable
        ...
        rows={rows.map((o): DataRow => ({
          ...
          cells: {
            ...
            kind: heldOutOfFleet(o.stage)
              ? <Pill tone={STAGE_TONE[stageOf(o.stage)]}>{STAGE_WORD[stageOf(o.stage)]}</Pill>
              : <Pill tone={o.kind === "provider" ? "warn" : "info"}>{o.kind}</Pill>,
            /* Their systems are on file and not in the fleet, and the roster is
               where somebody notices the difference. */
            has: (
              <span className="mut t-meta">
                {rosterSummary(o)}{heldOutOfFleet(o.stage) && o.systems > 0 ? " · not in the fleet" : ""}
              </span>
            ),
          },
        }))}

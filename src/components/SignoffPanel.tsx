...
export default function SignoffPanel({ target, ready, blockers, signatures, canSign, isOwner, myEmail, myName }: {
  ...
  return (
    <div className="card no-print">
      ...
      {!ready && (
        <div className="t-small" style={{ background: "#FAF0DC", color: "var(--t-warn-fg)", border: "1px solid #F0C9A0", borderRadius: 8, padding: "8px 12px", marginBottom: 10 }}>
      ...
      {signatures.map((s) => (
        <div key={s.id} style={{ borderTop: "1px solid var(--line)", padding: "8px 0" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
            <span className="t-lead" style={{ fontWeight: 700, fontFamily: "Georgia, serif", color: "var(--navy)" }}>{s.signerName}</span>
            {s.signerTitle && <span className="mut t-small">{s.signerTitle}</span>}
            <span className="pill good">{s.meaning}</span>
            <span className="mut t-meta" style={{ marginLeft: "auto" }}>{s.when}</span>
          </div>
          <div className="mut mono t-meta">{s.signedBy}</div>
          ...
          {s.stale && (
            <div className="t-meta" style={{ color: "var(--t-warn-fg)", marginTop: 3 }}>
              The record has changed since this was signed - the signature stands for the packet as it was that day.
            </div>
          )}
          {isOwner && (
            <button className="btn link" style={{ color: "var(--t-bad-fg)", padding: 0, marginTop: 3 }} disabled={pending}
              onClick={async () => {
      ...
      {signatures.length === 0 && (
        <div className="mut t-body">Nobody has signed this off yet.</div>
      )}
      ...
      {canSign && alreadySigned && (
        <div className="mut t-small" style={{ marginTop: 8 }}>You have signed this.</div>
      )}

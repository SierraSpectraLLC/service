...
var CreateAgentRedirect = function (meshserver, module, serverPublicNamePort, authCookie, rauthCookie, domainUrl) {
    ...
    obj.Start = function (nodeid) {
        // SIERRA MODIFICATION: the relay lives on a different host than this page,
        // so the socket address comes from obj.relayBase when set. Unset, this is
        // the original behaviour of deriving it from window.location.
        var base = (obj.relayBase != null) ? obj.relayBase
        ...
        var url2, url = base + '/' + obj.urlname + '?browser=1&p=' + obj.protocol + (nodeid?('&nodeid=' + nodeid):'') + '&id=' + obj.tunnelid;
